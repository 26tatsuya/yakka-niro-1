...
def create_map(filtered_rows):
    # 地図の初期設定
    map_center = [filtered_rows['latitude'].mean(), filtered_rows['longitude'].mean()]
    m = folium.Map(location = map_center, zoom_start=12)

    #マーカーを追加
    for idx, row in filtered_rows.iterrows():
        if rows.notnull(row['latitude'])and rows.notnull(row['longitude']):
...
def main():
    ...
    filtered_rows['latitude'] = pd.to_numeric(filtered_rows['latitude'], errors='coerce')
    filtered_rows['longitude'] = pd.to_numeric(filtered_rows['longitude'], errors='coerce')
    filtered_rows2 = filtered_rows.dropna(subset=['latitude', 'longitude'])
